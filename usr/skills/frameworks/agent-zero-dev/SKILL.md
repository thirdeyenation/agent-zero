...
| **Skills** | Create reusable instruction bundles | `usr/skills/custom/<skill-name>/` |
...
# Execute skill script (use runtime path from the load output)
await code_execution_tool.execute(
    runtime="python",
    code="python /a0/usr/skills/custom/my-skill/scripts/helper.py input"
)
```
