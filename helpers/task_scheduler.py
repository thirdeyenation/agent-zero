...
class AdHocTask(BaseTask):
    ...
    token: str = Field(default_factory=lambda: str(secrets.randbelow(9999999999999999999 - 1000000000000000000 + 1) + 1000000000000000000))
            ...
            for task in self.tasks:
                if isinstance(task, AdHocTask):
                    if task.token is None or task.token == "":
                        ...
                        task.token = str(secrets.randbelow(9999999999999999999 - 1000000000000000000 + 1) + 1000000000000000000)
                        PrintStyle.info(
    ...
    if not task_class:
        ...
        if task_type_str == 'scheduled':
            ...
            if not task_data.get('token'):
                task_data['token'] = str(secrets.randbelow(9999999999999999999 - 1000000000000000000 + 1) + 1000000000000000000)
